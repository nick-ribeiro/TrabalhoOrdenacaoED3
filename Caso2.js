...
length += July.length;
if (length >= 25) {
    July = July.filter(e => e !== 'Carol.Hamill42');
    July = July.filter(e => e !== 'Alverta.Herman');

    const countingSort = (July, n = July.length) => {
        let k = Math.max(...July);
        let t;
    ...
    July = countingSort(July);
    for (let i = 0; i < July.length; i++) {
        if (July[i] == 269) {
            July.splice(1, 0, 'Carol.Hamill42');
        }
        if (July[i] == 536) {
            August.splice(3, 0, 'Alverna.Herman');
        }
    }
